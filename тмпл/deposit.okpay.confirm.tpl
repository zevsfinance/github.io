...
Your OkPay account <b>{$account}</b><br>
Amount ({$currency_sign}): <b>{$amount_format}</b><br>
{if $use_compound}
<input type=hidden name=ok_item_1_custom_1_title value="compound" />
{if $compound_min_percents == $compound_max_percents && !$compound_percents}
<input type=hidden name='ok_item_1_custom_1_value' value="{$compound_min_percents}" />
{else}
<table cellspacing=0 cellpadding=2 border=0>
<tr><td nowrap width=1%>Compounding percent: </td>
  {if $compound_percents}
<td><select name='ok_item_1_custom_1_value' class=inpts>
{section name=p loop=$compound_percents}<option value="{$compound_percents[p].percent}">{$compound_percents[p].percent}%</option>{/section}
  ...
  {else}
<td width=99%><input type=text name='ok_item_1_custom_1_value' value="{$compound_min_percents}" class=inpts size=5></td></tr>
