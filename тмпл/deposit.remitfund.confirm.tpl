...
<form name=spend method=post action="https://www.remitfund.com/spend.php">

Your RemitFund account number <b>{$account}</b><br>
...
<br>
<INPUT type=hidden name=PAYEE_ACCOUNT value="{$settings.def_payee_account_remitfund}" >
<INPUT type=hidden name=PAYMENT_AMOUNT value="{$amount}">
<INPUT type=hidden name=CURRENCY value="USD">
<INPUT type=hidden name=INTERFACE value="{$settings.remitfund_interface}">
<INPUT type=hidden value="Deposit to {$settings.site_name} user {$userinfo.username}" name=SUGGESTED_MEMO>
<input type=hidden name=id value={$pid}>
<input type=hidden name=a value="remitfund_postback">
<input type=hidden name=BAGGAGE_FIELDS value="a,id, compound">
